...
def main():
    ...
    unet = apply_lora_to_model(pipe.unet).to(device, dtype)  # LoRA applied to both Conv2d and Linear layers
    tokenizer = pipe.tokenizer
    noise_scheduler = DDPMScheduler.from_config(pipe.scheduler.config)

    dataset = CustomDataset(r"/content/Dataset")
